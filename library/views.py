...
# from profiler import profile #profile is a decorator, but things get
# circular if I include it in decorators.py so...


def register(request):
    # TODO this is generic guff need to extend form for extra fields, send reg
    # data to transkribus and authticate (which will handle the user creation)
...
# return HttpResponse(json.dumps({'RESET': 'true', 'MESSAGE':
# render_to_string('libraryapp/message_modal.html', request=request)}),
# content_type='text/plain')
            except ValueError as err:
                sys.stdout.write(
                    "### t_register response ERROR RAISED: %s  \r\n" % (err))
#               return render(request, 'registration/register.html', {'register_form': form} )
                # Why the f**k won't this redirect?!? TODO fix or try another
                # method
...
def collections(request):
    ...
    collections = get_or_create_collections(request)
...
def collection(request, collId):
    # this is actually a call to collections/{collId}/list and returns only
    # the document objects for a collection

    get_or_create_collections(request)
    ...
    # The same could be achieved using the list of documents (ie pick first
    # doc match collId with member of colList)
    collection = None
...
def page(request, collId, docId, page):
    # call t_document with noOfTranscript=-1 which will return no transcript
    # data
    full_doc = t_document(request, collId, docId, -1)
    # big wodge of data from full doc includes data for each page and for each
    # page, each transcript...
    index = int(page) - 1
    # extract page data from full_doc (may be better from a  separate page
    # data request)
...
# transcripts ) )
    ...
    nav = navigation.up_next_prev("page", page, full_doc.get(
        "pageList").get("pages"), [collId, docId])
...
def transcript(request, collId, docId, page, transcriptId):
    ...
    nav = navigation.up_next_prev(
        "transcript", transcriptId, pagedata, [collId, docId, page])
...
def region(request, collId, docId, page, transcriptId, regionId):
    ...
    full_doc = t_document(request, collId, docId, -1)
    index = int(page) - 1
    # and then extract the correct page from full_doc (may be better from a
    # separate page data request??)
    pagedata = full_doc.get('pageList').get('pages')[index]
...
#    sys.stdout.write("############# PAGEDATA: %s\r\n" % pagedata )
    ...
    nav = navigation.up_next_prev("region", regionId, regions, [
                                  collId, docId, page, transcriptId])
...
def line(request, collId, docId, page, transcriptId, regionId, lineId):
    ...
    nav = navigation.up_next_prev("line", lineId, lines, [
                                  collId, docId, page, transcriptId, regionId])
...
def word(request, collId, docId, page, transcriptId, regionId, lineId, wordId):
    ...
    nav = navigation.up_next_prev("word", wordId, words, [
                                  collId, docId, page, transcriptId, regionId, lineId])
...
# This may be as simple as isPublished(), rather than any analysis on the
# content
...
def rand(request, collId, element):
    ...
    # best to avoid a random transcript, so we'll go for the current in the
    # hope that it is best....
    current_transcript = t_current_transcript(
        request, collId, doc.get("docId"), page.get("pageNr"))
    transcript = t_transcript(request, current_transcript.get(
    ...
    if regions:
        ...
        if transcript.get("PcGts").get("Page").get("TextLine"):
            # I don't think we ever get here.. need to check with UIBK if Page
            # > TextLine is even possible
...
def ingest_mets_xml(request):
    if request.method == 'POST':
        try:
            # if ingest_mets_xml_file_form.is_valid(): #  TODO Check something
            # for better error messages? And validate the file. Note: Django
            # allows form submission, even if no file has been selected.
            t_ingest_mets_xml(request.POST.get('collection'),
                              request.FILES['mets_file'])
...
def jobs_list(request):
    # TODO Consider making a form instead for persistence?
    if ('true' == request.POST.get('only_unfinished')):
        jobs = t_jobs(request, 'INCOMPLETE')
...
@t_login_required
# TODO Consider how much of a DOS risk these queries constitute.
def job_count(request):
    # I DO NOT KNOW why returning a JsonResponse or 'application/json' breaks
    # the cookies. The uncommented response below is what works. HttpFox
    # indicates that the only difference is text/plain vs. application/json
    # (or just json, both fail).
    ...
    # return HttpResponse(json.dumps({'CREATED': t_job_count('CREATED'),
    # 'FAILED': t_job_count('FAILED'), 'FINISHED':
    # t_job_count('FINISHED'),'WAITING': t_job_count('WAITING'), 'RUNNING':
    # t_job_count('RUNNING'), 'CANCELED': t_job_count('CANCELED'),
    # 'INCOMPLETE': t_job_count('INCOMPLETE')}),
    # content_type='application/json')
    return HttpResponse(json.dumps({'CREATED': t_job_count(request, 'CREATED'), 'FAILED': t_job_count(request, 'FAILED'), 'FINISHED': t_job_count(request, 'FINISHED'), 'WAITING': t_job_count(request, 'WAITING'), 'RUNNING': t_job_count(request, 'RUNNING'), 'CANCELED': t_job_count(request, 'CANCELED'), 'INCOMPLETE': t_job_count(request, 'INCOMPLETE')}), content_type='text/plain')
...
def jobs_list_compact(request):
    jobs = t_jobs(request)
    # TODO Decide what should be shown in the compact view. Only jobs which
    # have changed since some "acknowledgement"? Since the last login?
    # Since...?
    return render(request, 'libraryapp/jobs_list_compact.html', {'jobs': jobs})
...
def kill_job(request):
    ...
    # Leave as is or return an error message? The javascript could also handle
    # the error...
