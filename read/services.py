...
def get_or_create_collections(request):
    ...
    options = {'cookies': get_cookies(request)}

    collections = request.session.get('collections')

    if collections is None:
        collections = t_collections(**options)
...
def t_register(request):
    ...
    sys.stdout.write(
        "### [POST REQUEST] t_register POST to: %s with %s \r\n" % (url, params))
    sys.stdout.flush()
    r = s.post(url, params=params, verify=False, headers=headers)
    sys.stdout.write(
        "### t_register response STATUS_CODE: %s  \r\n" % (r.status_code))
    sys.stdout.write("### t_register response CONTENT: %s  \r\n" % (r.content))
    sys.stdout.flush()
    if r.status_code != requests.codes.ok:
        raise ValueError("Transkribus registration error",
...
#    headers = {'content-type': 'application/json'}
    ...
    sys.stdout.write(
        "### [POST REQUEST] t_login POST to: %s with %s \r\n" % (url, 'credentials'))
    sys.stdout.flush()
    r = s.post(url, params=params, verify=False, headers=headers)
    sys.stdout.write(
        "### t_login response STATUS_CODE: %s  \r\n" % (r.status_code))
    sys.stdout.flush()
    if r.status_code != requests.codes.ok:
        return None
    user_xml = r.text
...
# refresh transkribus session (called by t_login_required decorator to
# test persistence/validity of transkribus session)


def t_refresh(request):
    url = settings.TRP_URL + 'auth/refresh'

    options = get_options(request)

    sys.stdout.write(
        "### [POST REQUEST] t_refresh will POST: %s   \r\n" % (url))
    sys.stdout.flush()

    try:
        r = s.post(url, verify=False, **options)
    except requests.HTTPError as error:
        raise error

    sys.stdout.write(
        "### t_refresh response STATUS_CODE: %s   \r\n" % (r.status_code))
    sys.stdout.flush()
...
def t_collections(**options):
    ...
    sys.stdout.write(
        "### [GET REQUEST] t_collectionssss will GET: %s with %s \r\n" % (url, params))
    sys.stdout.flush()
    r = s.get(url, params=params, verify=False, headers=headers, **options)
    r.raise_for_status()
    if r.status_code != requests.codes.ok:
...
def t_collection(request, collId):
    ...
    # we will keep the current collection in the session to decrease the
    # number of calls transkribus.eu
    if "collection" in request.session:
        if request.session.get('collection')[0].get('cache_url') == url:
            sys.stdout.write(
                "### [HAVE CACHE] t_collection HAS CACHED collection for: %s \r\n" % (url))
            sys.stdout.flush()
    ...
    sys.stdout.write(
        "### [GET REQUEST] t_collectionnnnn will GET: %s with %s \r\n" % (url, params))
    sys.stdout.flush()
    r = s.get(url, params=params, verify=False, headers=headers, **options)
    if r.status_code != requests.codes.ok:
        sys.stdout.write("ERROR CODE: %s%% \r\n ERROR: %s%%" %
...
#            return request.session['doc']
    ...
    sys.stdout.write(
        "### [GET REQUEST] t_document will GET %s with %s \r\n" % (url, params))
    sys.stdout.flush()
...
def t_page(request, collId, docId, page, nrOfTranscripts=None):
    ...
    # we will keep the current page in the session to decrease the number of
    # calls to transkribus
    if "page" in request.session:
        if request.session.get('page')[0].get('cache_url') == url:
            sys.stdout.write(
                "### [HAVE CACHE] t_page HAS CACHED page for: %s \r\n" % (url))
            sys.stdout.flush()
    ...
    sys.stdout.write(
        "### [GET REQUEST] t_page will GET %s with %s \r\n" % (url, params))
    sys.stdout.flush()
    r = s.get(url, params=params, verify=False, headers=headers)
    if r.status_code != requests.codes.ok:
        return None
    ...
    for x in t_page:
        # key is used already for transcripts, however using key is handy for
        # jquery things like fancy tree...
        x['key'] = x.get('tsId')

    # Cache this to reduce calls on subsequent lower level web-pages
    # t_page is a list of objects, so where shall we keep the url.... in the
    # first object I guess
...
def t_current_transcript(request, collId, docId, page):
    ...
    if "current_transcript" in request.session:
        if request.session.get('current_transcript').get('cache_url') == url:
            sys.stdout.write(
                "### [HAVE CACHE] t_current_transcript HAS CACHED current_transcript for: %s \r\n" % (url))
            sys.stdout.flush()
    ...
    sys.stdout.write(
        "### [GET REQUEST] t_current_transcript will GET %s with %s \r\n" % (url, params))
    sys.stdout.flush()
    r = s.get(url, params=params, verify=False, headers=headers)
    if r.status_code != requests.codes.ok:
        return None
...
def t_transcript(request, transcriptId, url):
    ...
    # we will keep the current transcript in the session to decrease the
    # number of calls to retirve and parse pageXML
    if "transcript" in request.session:
        if request.session.get('transcript').get('cache_url') == url:
            sys.stdout.write(
                "### [HAVE CACHE] t_transcript HAS CACHED transcript for: %s \r\n" % (url))
            sys.stdout.flush()
...
def t_transcript_xml(request, transcriptId, url):
    # TODO Remove this? The xml is only used when saving after which the cache
    # must be refreshed anyway...
    if "transcript_xml" in request.session:
...
def t_save_transcript(request, transcript_xml, collId, docId, page):
    ...
    r = s.post(url, verify=False, headers=headers,
               params=params, data=transcript_xml)
...
def t_metadata(custom_attr):
    ...
    if not custom_attr:
        return None
    # CSS parsing (tinycss or cssutils) wasn't much cop so css => json by
    # homemade regex (gulp!)
...
def t_jobs(request, status=''):
    options = get_options(request)
    url = settings.TRP_URL + 'jobs/list'
    params = {'status': status}
    r = s.get(url, params=params, verify=False, **options)
    if r.status_code != requests.codes.ok:
...
def t_job_count(request, status=''):
    options = get_options(request)
