...
# override the login_required decorator, mostly so we can call
# services.t_collecctions at login
...
def t_login_required(function, redirect_field_name=REDIRECT_FIELD_NAME, login_url=None):
    ...
    def wrapper(request, *args, **kw):
        ...
        if request.user.is_authenticated():
            ...
            if not services.t_refresh(request):
            ...
            if "collections" not in request.session or request.session['collections'] is None:
                options = services.get_options(request)
                request.session[
                    'collections'] = services.t_collections(**options)
            return function(request, *args, **kw)
        else:
            path = request.build_absolute_uri()
            resolved_login_url = resolve_url(login_url or settings.LOGIN_URL)
            # If the login url is the same scheme and net location then just
            # use the path as the "next" url. #TODO fix this after port from
            # django source....
